...
ADMIN_ID = [int(x) for x in os.environ.get("ADMIN_ID", "7662192190,6041119040").split(",")]
DATABASE_URL = os.environ.get("DATABASE_URL", "")
READER_ID = [int(x) for x in os.environ.get("READER_ID", "7662192190,6041119040").split(",")]
...
async def group_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        "راهنمای سریع:\n"
        "• روی پیام شخصِ هدف «Reply» کرده و «نجوا / درگوشی / سکرت» بفرستید؛ سپس متن را در خصوصی ارسال کنید.\n"
        f"• حالت اینلاین: @{BOT_USERNAME or 'BgooOutis_Bot'} <متن> @username  یا فقط @{BOT_USERNAME or ''} برای نمایش مخاطبین اخیر."
    )
...
async def group_trigger(update: Update, context: ContextTypes.DEFAULT_TYPE):
   ...
   #        rows.append([InlineKeyboardButton("عضویت در کانال دو", url=f"https://t.me/{MANDATORY_CHANNELS[1]}")])
    ...
    guide = await context.bot.send_message(
        chat_id=chat.id,
        text=("لطفاً متن نجوای خود را در خصوصی ربات ارسال کنید: @{BOT}").format(BOT=BOT_USERNAME or ""),
        reply_to_message_id=msg.reply_to_message.message_id,
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✍️ ارسال متن در خصوصی", url=f"https://t.me/{BOT_USERNAME or 'BgooOutis_Bot'}?start=go")]])
    )
    async with pool.acquire() as con:
